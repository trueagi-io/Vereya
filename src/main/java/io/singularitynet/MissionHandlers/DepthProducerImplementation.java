...
public class DepthProducerImplementation extends HandlerBase implements IVideoProducer {
    ...
    public int[] writeFrame(MissionInit missionInit, ByteBuffer buffer) {
        ...
        int fbWidth = main != null ? main.textureWidth : width;
        int fbHeight = main != null ? main.textureHeight : height;
        ...
        int prevDrawBuf = GL11.glGetInteger(GL11.GL_DRAW_BUFFER);
        int prevReadBuf = GL11.glGetInteger(GL11.GL_READ_BUFFER);
        try {
    ...
    private void ensureFramebuffer(int width, int height) {
        if (this.fbo == null || this.fbo.textureWidth != width || this.fbo.textureHeight != height) {
            if (this.fbo != null) {
                this.fbo.delete();
            }
            this.fbo = new SimpleFramebuffer(width, height, true, MinecraftClient.IS_SYSTEM_MAC);
        }
    }
}

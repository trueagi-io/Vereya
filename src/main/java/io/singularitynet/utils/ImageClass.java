...
public final class ImageClass
    ...
    public ImageClass(int width, int height, boolean useStb) {
        this(Format.RGBA, width, height, useStb);
    }

    public ImageClass(Format format, int width, int height, boolean useStb) {
        if (width <= 0 || height <= 0) {
    ...
    private ImageClass(Format format, int width, int height, boolean useStb, long pointer) {
        if (width <= 0 || height <= 0) {
            throw new IllegalArgumentException("Invalid texture size: " + width + "x" + height);
        }
        this.format = format;
    ...
    public String toString() {
        return "ImageClass[" + this.format + " " + this.width + "x" + this.height + "@" + this.pointer + (this.isStbImage ? "S" : "N") + "]";
    }
    ...
    public static ImageClass read(InputStream stream) throws IOException {
        return ImageClass.read(Format.RGBA, stream);
    }

    /*
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    public static ImageClass read(@Nullable Format format, InputStream stream) throws IOException {
        ByteBuffer byteBuffer = null;
        try {
            byteBuffer = TextureUtil.readResource(stream);
            byteBuffer.rewind();
            ImageClass imageClass = ImageClass.read(format, byteBuffer);
            return imageClass;
        } finally {
    ...
    public static ImageClass read(ByteBuffer buffer) throws IOException {
        return ImageClass.read(Format.RGBA, buffer);
    }

    public static ImageClass read(@Nullable Format format, ByteBuffer buffer) throws IOException {
        ...
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ...
            ImageClass imageclass = new ImageClass(format == null ? Format.fromGl(intBuffer3.get(0)) : format, intBuffer.get(0), intBuffer2.get(0), true, MemoryUtil.memAddress(byteBuffer));
            return imageclass;
        }
    }
    ...
    public void blend(int x, int y, int color) {
        ...
        float f = (float)ImageClass.getAlpha(color) / 255.0f;
        float g = (float)ImageClass.getBlue(color) / 255.0f;
        float h = (float)ImageClass.getGreen(color) / 255.0f;
        float j = (float)ImageClass.getRed(color) / 255.0f;
        float k = (float)ImageClass.getAlpha(i) / 255.0f;
        float l = (float)ImageClass.getBlue(i) / 255.0f;
        float m = (float)ImageClass.getGreen(i) / 255.0f;
        float n = (float)ImageClass.getRed(i) / 255.0f;
        float o = f;
        float p = 1.0f - f;
        float q = f * o + k * p;
        float r = g * o + l * p;
        ...
        int w = (int)(s * 255.0f);
        int z = (int)(t * 255.0f);
        this.setColor(x, y, ImageClass.packColor(u, v, w, z));
    }
    ...
    public int[] makePixelArray() {
        ...
        for (int i = 0; i < this.getHeight(); ++i) {
            for (int j = 0; j < this.getWidth(); ++j) {
                ...
                int l = ImageClass.getAlpha(k);
                int m = ImageClass.getBlue(k);
                int n = ImageClass.getGreen(k);
                int o = ImageClass.getRed(k);
    ...
    private void uploadInternal(int level, int offsetX, int offsetY, int unpackSkipPixels, int unpackSkipRows, int width, int height, boolean blur, boolean clamp, boolean mipmap, boolean close) {
        RenderSystem.assertOnRenderThreadOrInit();
        this.checkAllocated();
        ImageClass.setTextureFilter(blur, mipmap);
    ...
    public void readDepthComponent(float unused) {
        ...
        if (this.format.getChannelCount() != 1) {
            throw new IllegalStateException("Depth buffer must be stored in ImageClass with 1 component.");
        }
        this.checkAllocated();
        this.format.setPackAlignment();
    ...
    public void copyFrom(ImageClass image) {
    ...
    public void copyRect(ImageClass image, int x, int y, int destX, int destY, int width, int height, boolean flipX, boolean flipY) {
    ...
    public void resizeSubRectTo(int x, int y, int width, int height, ImageClass targetImage) {
        this.checkAllocated();
    ...
    public static ImageClass read(String dataUri) throws IOException {
        byte[] bs = Base64.getDecoder().decode(dataUri.replaceAll("\n", "").getBytes(Charsets.UTF_8));
        try (MemoryStack memoryStack = MemoryStack.stackPush();){
            ByteBuffer byteBuffer = memoryStack.malloc(bs.length);
            byteBuffer.put(bs);
            byteBuffer.rewind();
            ImageClass imageclass = ImageClass.read(byteBuffer);
            return imageclass;
        }
    }

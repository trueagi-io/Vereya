...
void main() {
    ...
    // Fallback path: derive a single, stable colour per atlas sprite
    // using the integer grid cell from texCoord0. This ensures all
    // fragments of the same block texture share one segmentation colour.
    int grid = atlasGrid;
    if (grid <= 0) {
        grid = 32;
    }
    FragColor = colourFromAtlas(texCoord0, grid);
}
